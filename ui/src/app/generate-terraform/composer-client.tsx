...
export function ComposerClient() {
  ...
  const columnCounter = useRef(3);
  const [projectName, setProjectName] = useState("Untitled Architecture");
  const [columns, setColumns] = useState<string[]>(["col-1", "col-2"]);
  ...
  const generateTerraform = async () => {
    ...
    setBanner("Terraform generation wiring is next: file splitting, copy-all, and ZIP download.");
  };
